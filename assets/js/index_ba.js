...
const attachFeaturedMuseum = ({id, thumbnail, category, title, like, liked, view, link}) => {
...
const attachMuseum = ({id, thumbnail, category, title, like, liked, view, link}) => {
...
fetch('data.json')
  ...
  .then(data => {
    data.slice(0,24).map(({id, thumbnail, category, title, like, liked, view, link}, index) => attachMuseum({id, thumbnail, category, title ,like, liked, view, link}))
    data.filter(item => item.featured).map(({id, thumbnail, category, title, like, liked, view, link}, index) => attachFeaturedMuseum({id, thumbnail, category, title ,like, liked, view, link}))
  })
  ...
  return fetch('data.json')
      ...
      .then(data => {
        ...
        data
          ...
          .slice(museumsItem.length, museumsItem.length + thumbnailSize)
          .map(({id, thumbnail, category, title, like, liked, view, link}, index) => attachMuseum({id, thumbnail, category, title ,like, liked, view, link}))
        scrollToBottom()
        modalPreviewVirtualion()
      })
